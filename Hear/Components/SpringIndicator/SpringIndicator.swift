...
@IBDesignable class SpringIndicator: UIView {
    ...
    @IBInspectable var lineCap: Bool = false
    /// Rotation duration. Default is 1.5
    @IBInspectable var rotateDuration: Double = 1.5
...
extension SpringIndicator {
    ...
    func startAnimation(expand: Bool = false) {
    ...
    func stopAnimation(waitAnimation: Bool, completion: ((SpringIndicator) -> Void)? = nil) {
        if waitAnimation {
...
extension SpringIndicator {
    class Refresher: UIControl {
        ...
        let indicator = SpringIndicator(frame: CGRect(x: 0, y: 0, width: 20, height: 20))
        private(set) var refreshing: Bool = false
        private(set) var targetView: UIScrollView?
